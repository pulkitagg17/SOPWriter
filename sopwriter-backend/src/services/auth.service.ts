...
export class AuthService {
  ...
  async loginAdmin(email: string, password: string) {
    ...
    if (!admin) {
      await this.fakeHashCompare(password);
      await logAudit({
    ...
    const refreshToken = await this.issueRefreshToken(admin);
  ...
  async forgotPassword(email: string, requestId: string) {
    ...
    log.info({ adminId: admin._id }, 'OTP sent successfully');
  }
  ...
  async refreshSession(token: string) {
    ...
    const newRefreshToken = await this.issueRefreshToken(admin);
  ...
  // PRIVATE HELPERS
  // ============================
  private generateAccessToken(admin: any) {
    return jwt.sign(
      {
  ...
  private async issueRefreshToken(admin: any) {
  ...
  private async fakeHashCompare(password: string) {
